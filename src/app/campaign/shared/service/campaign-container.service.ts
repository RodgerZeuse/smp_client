...
export class CampaignContainerService {
        ...
        return this.campaignService.createCampaign(campaign)
            ...
            .flatMap((imageFilesRes: Array<MediaFileAsset>) => {
                ...
                if (imageFilesRes && imageFilesRes.length >= 1) {
                    UploadedServerFiles = this.returnMediaFilesNames(campaignVM.mediaFiles, imageFilesResult);
            ...
            .flatMap((uploadedThumbnails: Array<MediaFileAsset>) => {
                ...
                if (uploadedThumbnails && uploadedThumbnails.length >= 1) {
                    uploadedFilesWithThumbnails = this.updatemediaFilesWiththumbnails(uploadedThumbnails, UploadedServerFiles)
                }
                return this.campaignService.updateCamapign(campaign.id, uploadedFilesWithThumbnails);
            })
            .flatMap((campaignRes: Campaign) => {
                campaign = campaignRes;
                if (campaign.mediaFiles.length > 0 && campaignVM.posts.length > 0) {
                    postsArray = this.postsWithFiles(uploadedFilesWithThumbnails, campaignVM);
                    ...
                    campaignVM.posts.forEach(post => {
                        ...
                        if (campaignVM.campaignStatuses && campaignVM.campaignStatuses.id) {
                            postForServer.statusId = campaignVM.campaignStatuses.id;
                        }
            ...
            .flatMap((croppedImageFilesRes: Array<MediaFileAsset>) => {
                ...
                if (croppedImageFilesResult && postsArray.length > 0) {
                    this.returnCroppedMediaFilesNames(croppedImageFilesResult, postsArray);
                }
    ...
    updateCampaign(campaignVM: CampaignViewModel) {
        ...
        return this.campaignService.removeMediaFilesfromContainer(campaignVM.deletedPersistantImages)
            ...
            .flatMap((imageFilesRes: Array<MediaFileAsset>) => {
                imageFilesResult = [...imageFilesRes];
                if (imageFilesRes && imageFilesRes.length >= 1) {
                    UploadedServerFiles = this.returnMediaFilesNames(mediaFilesToUpload, imageFilesResult);
                    return this.campaignService.uploadImage(thumbnails);
                } else {
                    return Observable.of([]);
                }
            ...
            .flatMap((uploadedThumbnails: Array<MediaFileAsset>) => {
                ...
                if (uploadedThumbnails && uploadedThumbnails.length >= 1) {
                    uploadedFilesWithThumbnails = this.updatemediaFilesWiththumbnails(uploadedThumbnails, UploadedServerFiles)
                    campaign.mediaFiles = campaign.mediaFiles.concat(uploadedFilesWithThumbnails);
                }
                ...
                if (campaignVM.posts.length >= 1) {
                    let persistAndNewPosts = this.replaceImagesPostsEditCase(campaign, campaignVM, uploadedFilesWithThumbnails)
                ...
                if (croppedImageFilesResult && allPostToUpdateMediaFiles.length > 0) {
                    this.returnCroppedMediaFilesNames(croppedImageFilesResult, allPostToUpdateMediaFiles);
                }
    ...
    toDeleteCroppedImagesFromDeletedPosts(campaignVM: CampaignViewModel) {
        if (campaignVM.postsToDelete.length > 0) {
            campaignVM.postsToDelete.forEach(post => {
                let imageToDelete = post.mediaFiles.find(x => x.isCropped == true && x.id && x.id != '');
                if (imageToDelete) {
                    let imageFoundInOtherPost = _.find(campaignVM.posts, _.flow(
                        _.property('mediaFiles'),
                        _.partialRight(_.some, { name: imageToDelete.name, isCropped: true})
                    ));
    ...
    replaceImagesPostsEditCase(campaign, campaignVM, uploadedFilesWithThumbnails) {
        let persistPostArray: Array<Post> = [];
        let newPostsArray: Array<Post> = [];
        ...
        campaignVM.posts.forEach((post: PostViewModel) => {
            ...
            if (campaign.campaignStatuses && campaign.campaignStatuses.id) {
                postForServer.statusId = campaign.campaignStatuses.id;
            }
            if (post.id && post.id != '') {
                if (uploadedFilesWithThumbnails.length >= 1) {
                    uploadedFilesWithThumbnails.forEach(file => {
                        if (((postForServer.type == 'video' || postForServer.type == 'multiple-videos') && file.type == 'video/mp4') ||
                            ((postForServer.type == 'image' || postForServer.type == 'multiple-images') && file.type != 'video/mp4')) {
                ...
                if (post.mediaFiles.length >= 1) {
                    post.mediaFiles.forEach(file => {
                        if (file.id && file.id != '') {
                            if (((postForServer.type == 'video' || postForServer.type == 'multiple-videos') && file.type == 'video/mp4') ||
                                ((postForServer.type == 'image' || postForServer.type == 'multiple-images') && file.type != 'video/mp4')) {
                                postForServer.mediaFiles.push(Object.assign({}, file.toServerModel()));
                            }
                        }
                    })
                }
    ...
    postsWithFiles(filesWiththumbnails: Array<MediaFileAsset>, campaignVM: CampaignViewModel, ) {
        let postsArray: Array<Post> = [];
        campaignVM.posts.forEach(post => {
            let postForServer: Post = new Post();
            postForServer = post.toServerModelWithoutFiles();
            ...
            if (filesWiththumbnails.length >= 1) {
                filesWiththumbnails.forEach(file => {
                    if ((file.type == 'video/mp4' && (postForServer.type == 'video' || postForServer.type == 'multiple-videos'))
                        || (file.type != 'video/mp4' && (postForServer.type == 'image' || postForServer.type == 'multiple-images')))
                        postForServer.mediaFiles.push(Object.assign({}, file));
                })
            }
            postsArray.push(postForServer);
    ...
    returnMediaFilesNames(mediaFiles, fileUploadRes) {
        ...
        mediaFiles.forEach(persistFiles => {
            let imageFoundIndex = files.findIndex(x => x.originalFilename == persistFiles.file.name);
    ...
    updatemediaFilesWiththumbnails(uploadedThumbnails, mediafiles) {
        ...
        let files = _.map(result1.result.files.uploadFile, _.partialRight(_.pick, ['originalFilename', 'name', 'type']));
        mediafiles.forEach(persistFiles => {
            let imageFoundIndex = files.findIndex(x => x.originalFilename.substr(0, x.originalFilename.lastIndexOf('.')) == persistFiles.originalFilename.substr(0, persistFiles.originalFilename.lastIndexOf('.')));
            if (imageFoundIndex > -1) {
    ...
    returnCroppedMediaFilesNames(uploadedImages, postArray: Array<Post>) {
        ...
        uploadedImages.forEach(UPImage => {
            ...
            imagesArray = imagesArray.concat(_.map(result1.data, _.partialRight(_.pick, ['originalFilename', 'name', 'type', 'network'])));
        })
        imagesArray.forEach((image: MediaFileAsset) => {
            postArray.forEach((post, cIndex) => {
                let imageFound = post.mediaFiles.findIndex(x => x.originalFilename == image.originalFilename && (image.network == post.network || image.network == 'All'))
                if (imageFound > -1) {
    ...
    onChangeDescription(description: string, campaignViewModel: CampaignViewModel) {
    ...
    removeMediaFileFromCampaignViewModel(incomingFile, campaignViewModel: CampaignViewModel) {
        if (incomingFile.id != '' && incomingFile.id) {
            let findIndex = campaignViewModel.mediaFiles.findIndex(x => x.originalFilename == incomingFile.originalFilename);
            if (findIndex > -1) {
                campaignViewModel.mediaFiles.splice(findIndex, 1);
                campaignViewModel.deletedPersistantImages.push(incomingFile.name);
                campaignViewModel.deletedPersistantImages.push(incomingFile.thumbnailName);

            }
        } else {
            let findIndex = campaignViewModel.mediaFiles.findIndex(x => x.name == incomingFile.name);
            if (findIndex > -1) {
                campaignViewModel.mediaFiles.splice(findIndex, 1);
            }
        }
    ...
    setCampaignTypeRelatedtoMediaFiles(campaign: CampaignViewModel) {
        if (campaign.mediaFiles.length == 0) {
            ...
            campaign.type = (campaign.mediaFiles[0].type == 'vide/mp4' ? 'video' : 'image')
        } else if (campaign.mediaFiles.length > 1) {
    ...
    removeMediaFileFromPost(incomingFile, campaignViewModel: CampaignViewModel) {
        ...
        campaignViewModel.posts.forEach((post, index) => {
            let findIndex = -1
            if (incomingFile.id != '' && incomingFile.id) {
                findIndex = post.mediaFiles.findIndex(x => x.originalFilename == incomingFile.originalFilename);
                if (findIndex == -1) {
                    findIndex = post.mediaFiles.findIndex(x => x.name == incomingFile.originalFilename);
                }
            } else {
                findIndex = post.mediaFiles.findIndex(x => x.name == incomingFile.name);
            }
            if (findIndex > -1) {
                if (post.mediaFiles[findIndex].isCropped && post.mediaFiles[findIndex].id) {
                    campaignViewModel.deletedPersistantImages.push(post.mediaFiles[findIndex].name);
                }
    ...
    setPostTypeRelatedToMediaFile(post) {
        if (post.mediaFiles.length > 1) {
            post.type = (post.mediaFiles[0].type == 'video/mp4' ? 'multiple-videos' : 'multiple-images')
    ...
    addMediaFileInCampaign(newFile: MediaFileAssetViewModel, campaignViewModel: CampaignViewModel) {
        let fileFound = campaignViewModel.mediaFiles.find(x => x.name == newFile.name);
        if (fileFound) {
            ...
            if (newFile.thumbnailFile) {
                ...
                file.thumbnailFile.base64File = Object.assign({}, newFile.thumbnailFile.file);

            }
            campaignViewModel.mediaFiles.push(Object.assign({}, newFile));
            campaignViewModel.type = this.setCampaignTypeRelatedtoMediaFiles(campaignViewModel);
    ...
    addMediaFileInToRelativePost(newFile: MediaFileAssetViewModel, post: PostViewModel) {
    ...
    uniquSelectedSocialNetworks(selectedSocialAccountsArtifactsArray:Array<SocialAccountForCreate>) {
    ...
    onAddMediaFile(newFile: MediaFileAssetViewModel, campaignViewModel: CampaignViewModel) {
        let fileFound = campaignViewModel.mediaFiles.find(x => x.name == newFile.name);
        if (fileFound) {
            fileFound = Object.assign({}, newFile);
        } else {
        ...
        campaignViewModel.posts.forEach(post => {
            ...
            if (!fileFound) {
                ...
                file.croppedImages = [];
                if ((post.type == 'video/mp4' && file.type == 'video/mp4') || (post.type != 'video/mp4' && file.type != 'video/mp4') || post.type == 'text') {
    ...
    fillCampaignAndPostWithCroppedImage(incomingCroppedfile: MediaFileAssetViewModel, campaignViewModel: CampaignViewModel) {


        let existingImageFile = campaignViewModel.mediaFiles.find(x => (x.originalFilename == incomingCroppedfile.name || x.name == incomingCroppedfile.name));
        if (existingImageFile != null) {
            let existingCroppedImage = null;
            if (existingImageFile.croppedImages && existingImageFile.croppedImages.length > 0) {
                existingCroppedImage = existingImageFile.croppedImages.find(x =>
                    (x.originalFilename == incomingCroppedfile.name || x.name == incomingCroppedfile.name)
                    && (x.network == incomingCroppedfile.network || incomingCroppedfile.network == 'All'))
        ...
        campaignViewModel.posts.forEach((post, index) => {
            ...
            let existingCroppedFile = post.mediaFiles.find(x => (x.originalFilename == incomingCroppedfile.name || x.name == incomingCroppedfile.name) && (post.network == incomingCroppedfile.network || incomingCroppedfile.network == 'All'));
            if (existingCroppedFile) {
                if (existingCroppedFile.id && existingCroppedFile.isCropped) {
                    campaignViewModel.deletedPersistantImages.push(existingCroppedFile.name);
    ...
    onAddOrRemoveTags(image, campaignViewModel: CampaignViewModel) {
        if (image.id && image.id != '') {
            let findIndex = campaignViewModel.mediaFiles.findIndex(x => x.originalFilename == image.originalFilename);

            if (findIndex > -1) {
                campaignViewModel.mediaFiles[findIndex].tags = Object.assign([], image.tags);
            }
            campaignViewModel.posts.forEach(post => {
                let findIndex = post.mediaFiles.findIndex(x => x.originalFilename == image.originalFilename);
                if (findIndex > -1) {
            ...
            let findIndex = campaignViewModel.mediaFiles.findIndex(x => x.name == image.name);
    ...
    removeThumbnailsFromMediaFilesViewModel(files: Array<MediaFileAssetViewModel>) {
        files.forEach(file => {
    ...
    makePostOnAddnewNetwork(selectedArtifact: SocialAccountForCreate,
        campaignViewModel: CampaignViewModel, selectedArtifactsArray: Array<SocialAccountForCreate>) {
        let ifImageExist = campaignViewModel.mediaFiles.some(x => x.type != 'video/mp4');
        let ifVideoExist = campaignViewModel.mediaFiles.some(x => x.type == 'video/mp4');
        let ifMultipleImages = campaignViewModel.mediaFiles.filter(x => x.type != 'video/mp4');
        let ifMultipleVideos = campaignViewModel.mediaFiles.filter(x => x.type == 'video/mp4');
        ...
        noOfPosts.forEach(postType => {
            ...
            postViewObj.error = "";
            postViewObj.type = (postType == 'text' ? 'text' : postType == 'image' && ifMultipleImages.length == 1 ? 'image' :
                postType == 'image' && ifMultipleImages.length > 1 ? 'multiple-images' : postType == 'video' && ifMultipleVideos.length == 1 ? 'video' : postType == 'video' && ifMultipleVideos.length > 1 ? 'multiple-videos' : 'error');
    ...
    removeOneSelectedArtifactRelatedToPost(post: PostViewModel, selectedSocialAccountsArtifactsArray: Array<SocialAccountForCreate>) {
        let findIndex = selectedSocialAccountsArtifactsArray.findIndex(x => x.socialAccountObj.id
            == post.socialMediaAccountId && x.pageObj.id == post.socialMediaInfo.id);
    ...
    fileRead(isConvertToFile, isConvertToBase64, files: Array<MediaFileAssetViewModel>) {
        ...
        files.forEach(async (serverfile: MediaFileAssetViewModel, index) => {
            ...
            if (isConvertToBase64) {
                promises.push(this.mediaAssetConversionUtil.serverFileToBase64(serverfile))
                console.log('base64 conversion', new Date());
            ...
            if (isConvertToFile) {
                ...
                //     promises.push(this.mediaAssetConversionUtil.base64ToFile(serverfile.base64File))
                //     // serverfile.file = await this.mediaAssetConversionUtil.base64ToFile(serverfile.base64File)
                //     console.log('file conversion1', new Date())
                ...
                //     cFile.name = serverfile.originalFilename;
                //     cFile.type = serverfile.type;
                //     promises.push(this.mediaAssetConversionUtil.base64ToFile(cFile));
                //     console.log('file conversion2', new Date());
                // }
    ...
    deletePostsToShow(socialAccountsArtifact: SocialAccountForCreate, campaignViewModel: CampaignViewModel) {
        // let postsToDelete = [];
        let loopLength = campaignViewModel.posts.length;
        for (let i = 0; i < loopLength; i++) {
            let findIndex = campaignViewModel.posts.findIndex(x =>
            ...
            if (findIndex > -1) {
                if (campaignViewModel.posts[findIndex].id && campaignViewModel.posts[findIndex].id != '') {
                    campaignViewModel.postsToDelete.push(campaignViewModel.posts[findIndex]);
    ...
    deletePostContainThisFile(post: PostViewModel, incomingFile: MediaFileAssetViewModel, campaignViewModel: CampaignViewModel) {
        let postToDelete = campaignViewModel.posts.findIndex(x => x.mediaFiles.find(x => x.type == incomingFile.type) &&
            x.socialMediaAccountId == post.socialMediaAccountId && x.socialMediaInfo.id == post.socialMediaInfo.id);

        if (postToDelete > -1) {
            if (post.id) {
                campaignViewModel.postsToDelete.push(campaignViewModel.posts[postToDelete]);
            }
    ...
    makePostForViewModelOnAddNewTypeFile(newFile: MediaFileAssetViewModel, selectedArtifact: SocialAccountForCreate, campaignViewModel: CampaignViewModel) {
    ...
    assignScheduleDate(campaign) {
        ...
        if (campaign.state == "In Queue") {
            return this.campaignService.getAvailableSlot().map(date => {
    ...
    // createPostsForEachSocialAccount(postsArray, createdCampaign) {
    //     return this.campaignService.createPosts(postsArray, createdCampaign).map(res => {
    ...
    //     })
    // }
    // uploadingCroppedMediaFiles(croppedFiles: Array<MediaFileAssetViewModel>, postArray: Array<Post>) {
    //         return this.campaignService.uploadCroppedImage(croppedFiles).map(res => {
    //             return res
    //         }, err => {
    //             return err;
    //         })
    ...
    // uploadingMediaFiles(mediaFiles) {
    //     if (mediaFiles.length >= 1) {
    //         return this.campaignService.uploadImageNew(mediaFiles).map(res => {
    ...
    //         return this.returnUpdatedMediaFiles(mediaFiles, res);
    ...
    // returnUpdatedMediaFiles(mediaFiles, res) {
    ...
    //             files[imageFoundIndex].tags = persistFiles.tags;
    //             files[imageFoundIndex].isCropped = false;
    //             files[imageFoundIndex].id = UUID.UUID();
