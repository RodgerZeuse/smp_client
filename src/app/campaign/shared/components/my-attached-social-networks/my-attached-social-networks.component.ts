...
export class MyAttachedSocialNetworksComponent implements OnInit {
  ...
  @Output() onSelectSocialAccountArtifact: EventEmitter<any> = new EventEmitter<
    any
  >();
  ...
  @Output() onRemoveAccountsPage: EventEmitter<
    Array<SocialAccountForCreate>
  > = new EventEmitter<Array<SocialAccountForCreate>>();
  @Input() selectedSocialAccountsArtifactsArray$: Observable<
    Array<SocialAccountForCreate>
  >;
  socialAcountObj: SocialAccountForCreate;
  isAttachedAccountExist: boolean;
  @Output() resetSeletedAccount: EventEmitter<boolean> = new EventEmitter<
    boolean
  >();
  ...
  checkAccountPageExist(accountId: string) {
    let VALID = {};
    this.selectedSocialAccountsArtifactsArray$.subscribe((res: any) => {
      console.log(res);
      VALID = res;
    });
    return _.filter(VALID, {
