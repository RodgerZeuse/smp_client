...
export class CampaignCalendarComponent implements OnInit, OnChanges {
      ...
      return {
        ...
        color:
          campaign.state == "Draft"
            ? this.campaignCalendar.eventColors.yellow
            : campaign.state == "Scheduled"
            ? this.campaignCalendar.eventColors.blue
            : campaign.state == "Posted"
            ? this.campaignCalendar.eventColors.green
            : campaign.state == "InQueue"
            ? this.campaignCalendar.eventColors.pink
            : this.campaignCalendar.eventColors.red,
        actions:
          campaign.state == "Posted" &&
          campaign["ownerId"] == this.profileStore.userId
            ? this.actionsForPosted
            : campaign.state == "Canceled"
            ? this.actionsForCanceled
            : this.profileStore.userAccountType == "company" ||
              campaign["ownerId"] == this.profileStore.userId
            ? this.actionsForAllRights
            : campaign["ownerId"] == this.profileStore.userId
            ? this.actionsToDeleteCampaigns
            : campaign["ownerId"] == this.profileStore.userId
            ? this.actionsToEditCampaigns
            : this.actionsForNone,
        draggable:
          campaign.state == "Posted" || campaign.state == "Canceled"
            ? false
            : true
      };
    });
  ...
  editCampaign(event) {
    this.router.navigate(["/campaign/edit-campaign"], {
    ...
    this.campaigns.forEach(cam => {
      if (cam.id == event.id) {
        if (cam.state == "Posted" || cam.state == "Canceled") {
          ...
          this.snackBar.open(
            cam.state + " is not allowed to reschedual",
            "Ok",
            { duration: 2000 }
          );
        } else if (event.meta["ownerId"] == this.profileStore.userId) {
          ...
          this.snackBar.open("You are not allowed to reschedual", "Ok", {
            duration: 2000
          });
        }
      }
    ...
    this.campaigns.forEach(cam => {
      if (cam.id == event.id) {
        if (cam.state == "Posted" || cam.state == "Canceled") {
          this.postedOrCanceled.isDragged = true;
          this.postedOrCanceled.type = cam.state;
          this.snackBar.open(cam.state + " is not allowed to drag", "Ok", {
            duration: 2000
          });
        } else if (event.meta["ownerId"] == this.profileStore.userId) {
  ...
  showDayEvents(day: CalendarMonthViewDay): void {
    if (isSameMonth(day.date, this.campaignCalendar.currentViewDate)) {
      ...
      if (
        (isSameDay(this.campaignCalendar.currentViewDate, day.date) &&
  ...
  deleteDialogBox(event) {
    ...
    dialogConfig.data = {
      ...
      description:
        errors.deleteCampaignPopup +
        this.campaigns.find(x => x.id == event.id).title,
