...
export class SocialMediaService {
  ...
  twitterOauth() {
    return this.http.get("http://d1c4432b.ngrok.io/api/SocialMediaAccounts/session/connect").map(res => {
      return res;
    })
  }
  ...
  createPinterestAccount(session, profile, boards) {
    ...
    let ac = {
      type: 'PI', status: 'connected', userId: profile.data.id,companyId:this.profileStore.company.id, userName: profile.data.username ? profile.data.username : profile.data.first_name, session: session,
      registerUserId: this.userId, pictureThumbnail: profile.data.image ? profile.data.image["60x60"].url : this.icons.avatarIcon, boards: boards, profile: profile
    };
  ...
  createFacebookAccount(id, name, session, pictureThumbnail) {
    ...
    let ac = {
      type: 'FB', userId: id,companyId:this.profileStore.company.id, userName: name, session: session.authResponse, status: session.status,
      registerUserId: this.userId, pictureThumbnail: pictureThumbnail
    };
    return this.socialMediaAccountApi.create(ac).map((account) => {
      return account;
    }, err => {
  ...
  createInstagramAccount(id, name, session, pictureThumbnail) {
    ...
    // headers.append('Authorization', `Bearer ${this.access_token}`);
    let ac = {
      type: 'IG', userId: id, userName: name,companyId:this.profileStore.company.id, session: session.authResponse, status: session.status,
      registerUserId: this.userId, pictureThumbnail: pictureThumbnail
    };
    return this.socialMediaAccountApi.create(ac).map((account) => {
      return account;
  ...
  getLinkedinAccessToken(code,state){
    ...
    return this.http.get("https://www.linkedin.com/oauth/v2/accessToken?grant_type=authorization_code&code="+ code +"&redirect_uri=http://localhost:4200/my-networks&client_id=77g7k13xjls3dk&client_secret=dTez37vyK56uuERH",options).map(res =>{
       
    return res;
    })
  }
  getPagesAndGroups(id: string) {
    return this.http.get("http://192.168.1.127:3003/api/SocialMediaAccounts/getSmPages?socialMediaId=" + id, {}).map(res => {
      return res;
    })
