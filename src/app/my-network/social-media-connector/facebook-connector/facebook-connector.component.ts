...
export class FacebookConnectorComponent {
  ...
  constructor(private ref: ChangeDetectorRef, private dialog: MatDialog, 
    public socialMediaService: SocialMediaService, public userProfile: UserProfile, 
  ...
  facebookAuth() {
    ...
    FB.login(async (response) => {
      if (response.authResponse) {
        FB.api('/me', { fields: 'id,name,email,picture' }, async resp => {
          try {
            self.socialMediaService.createFacebookAccount(resp.id, resp.name, response, resp.picture.data.url).subscribe((account: any) => {
        ...
        scope: 'publish_pages,manage_pages,user_likes,user_status,email,pages_show_list'
      });
  }
